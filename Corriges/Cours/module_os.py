...
def main():
    ...
    GET_MTIME = path.getmtime(PATH_JOIN)
    print(f"Date de modification du fichier : {time.ctime(GET_MTIME)}")
    print(f"Date de modification du fichier : {datetime.datetime.fromtimestamp(GET_MTIME)}")
    print()
    
    # Caclucler le temps écoulé depuis la dernière modification
    td = datetime.datetime.now() - datetime.datetime.fromtimestamp(GET_MTIME)
